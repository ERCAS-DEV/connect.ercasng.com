...
class Tickets_model extends CI_Model {
	...
	public function ticket_listing($id){
		$query = $this->db->where(array('biller_id'=>$id))->order_by("creation_date", "desc")->get('tickets')->result();
		return $query;		
	}	
	/******** EOF *******/
	...
	public function get_open_tickets_count($biller_id){
		$query = $this->db->where(array('status'=>'Pending','biller_id'=>$biller_id))->order_by("resolving_date", "desc")->get('tickets')->result();
		return sizeof($query);
	}
}
