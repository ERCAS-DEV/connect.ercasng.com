...
class Biller_model extends CI_Model {
    ...
    function login($data)
    {
      $sql    = "select * from biller where email_address='".$data['email']."' and password='".sha1(md5($data['password']))."' and status='1'";
	...
	/*** EOF ***/
	/***  Function for Biller Subuser Exists or not ***/
	public function biller_subuser_exists($email_address,$biller_username){		
		$sql = "select * from biller_subuser where email = '".$email_address."' or username = '".$biller_username."'";
		$query  = $this->db->query($sql);
		return $query->num_rows();
	}
	/****** EOF *****/
	...
	public function subuser_registration($data){
		$this->db->insert('biller_subuser', $data);	
		return $this->db->insert_id();
	}
	/****** EOF *****/
	/***  Function for Biller Subusers ***/
	public function biller_subusers_listing(){
		$query = $this->db->get('biller_subuser')->result();
		return $query;	
	}

	/***** Function for Edit Biller Subuser****/
	public function billersubuser_detail($id){
		$query = $this->db->where(array('id'=>$id))->get('biller_subuser')->result();
		return $query;
	}
	/****** EOF *****/
