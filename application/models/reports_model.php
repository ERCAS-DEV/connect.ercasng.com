...
class Reports_model extends CI_Model {
	...
	public function generate_analysis_data($tblnm,$startdate,$enddate){
		...
		$data = $this->db->query("select *, UNIX_TIMESTAMP( STR_TO_DATE( TransDate,  '%Y-%m-%d %H:%i' ) ) as timestampdate, str_to_date(TransDate,'%Y-%m-%d %H:%i') as datetimestamp FROM $tblnm where str_to_date(TransDate,'%Y-%m-%d %H:%i')>='".$startdate."' and str_to_date(TransDate,'%Y-%m-%d %H:%i') <='".$enddate."'")->result();
